...
|`user` | Both | The user to use for the connection. | `root` when using type `ssh` and defaults to `Administrator` when using type `winrm`. |
...
| `port` | Both| The port to connect to. | `22` when using type `ssh` and defaults to `5985` when using type `winrm`. |
